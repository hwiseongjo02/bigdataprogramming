...
대용량 원본 데이터와 분석 결과물은 GitHub에 직접 커밋하지 않고, 수집 및 분석 스크립트를 통해 재생성하는 것을 원칙으로 한다.
...
Kaggle API를 사용하려면 Kaggle 계정의 `kaggle.json` 인증 파일이 필요하다. 이 파일은 개인 인증 정보이므로 GitHub에 업로드하지 않는다.
...
이미 인증 파일이 있는 경우에는 복사 단계를 생략해도 된다.
...
HDP Sandbox에서 `python`이 Python 2.7을 가리키는 경우가 있으므로 Python 3.6을 직접 사용한다.
...
`--max-chunks 0`은 각 연도 파일의 모든 청크를 사용한다는 의미이다. 빠른 테스트만 하고 싶다면 `--max-chunks 5`처럼 값을 줄일 수 있다.
...
Python 실행 파일을 직접 지정하고 싶다면 다음처럼 실행한다.

```bash
USE_HDFS=1 PYTHON_BIN=/usr/bin/python3.6 bash run_pipeline.sh
```

빠른 테스트용으로 청크 수를 줄이고 싶다면 다음처럼 실행한다.

```bash
USE_HDFS=1 PYTHON_BIN=/usr/bin/python3.6 MAX_CHUNKS=5 bash run_pipeline.sh
```
...
### 시각화 PNG에서 한글이 깨지는 경우

샌드박스에 한글 폰트가 없으면 PNG의 한글이 깨질 수 있다. 실행 성공 여부와 분석 결과 CSV 생성에는 영향이 없다. 보고서나 발표자료에는 한글 폰트가 설치된 로컬 환경에서 생성한 그래프를 사용하는 것을 권장한다.

...
HDFS_RAW_DIR = f"/user/{USER}/brfss/raw"
...
def main():
    ...
    if args.skip_download:
        ...
        print(f"Kaggle에서 데이터셋({DATASET}) 다운로드받는 중")
        api = KaggleApi()
        api.authenticate() 
        api.dataset_download_files(DATASET, path=str(save_path), unzip=False)

        zip_file = save_path / f"{DATASET.split('/')[-1]}.zip"
        print("압축 푸는 중")
    ...
    for csv_file in csv_files:
        ...
        for i, chunk in enumerate(chunk_iterator):
            ...
            output_file = save_path / f"part_{year_str}_{i+1}.csv"
            chunk.to_csv(output_file, index=False)
            print(f"  -> 저장완료: {output_file.name} ({len(chunk)}개)")

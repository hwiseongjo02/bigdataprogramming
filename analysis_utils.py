...
def save_bmi_distribution(df, output_dir, sample_size):
    bmi_data = collect_bmi_sample(df, sample_size)
    ...
    draw_bmi_boxplot(bmi_data, output_dir / "plot3_bmi.png")


def collect_bmi_sample(df, sample_size):
    return (
        df.filter((col("BMI_Real") > BMI_MIN) & (col("BMI_Real") < BMI_MAX))
        .select("Has_Disease", "BMI_Real")
        .sample(False, 0.1, seed=RANDOM_SEED)
        .limit(sample_size)
        .toPandas()
    )


def draw_bmi_boxplot(bmi_data, file_path):
    use_presentation_style()
...
def make_model_dataset(df):
    ...
    return model_df.filter(
        ...
        & (col("BMI_Real").between(BMI_MIN, BMI_MAX))
        & (col("SEX").isin([1, 2]))
    )
...
def save_model_coefficients(model, output_dir):
    coef_df = build_coefficient_frame(model)
    ...
    draw_model_coefficients(coef_df, output_dir / "plot7_model_coefficients.png")


def build_coefficient_frame(model):
    rows = [
        {
            "feature": feature,
    ...
    return pd.DataFrame(rows).sort_values("coefficient", ascending=False)


def draw_model_coefficients(coef_df, file_path):
    use_presentation_style()
...
def split_model_data(model_df):
    return model_df.randomSplit([TRAIN_RATIO, TEST_RATIO], seed=RANDOM_SEED)


def vectorize_model_data(train_df, test_df):
...
def fit_logistic_regression(train_vec):
...
def run_logistic_regression(df, output_dir):
    ...
    if model_count == 0:
        print("모델 학습에 사용할 데이터가 없습니다.")
        return

    train_df, test_df = split_model_data(model_df)
    train_vec, test_vec = vectorize_model_data(train_df, test_df)
    model = fit_logistic_regression(train_vec)
